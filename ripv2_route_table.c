 ...
 *   crear la ruta.
 */
ripv2_route_t * ripv2_route_create(ipv4_addr_t subnet, ipv4_addr_t mask, char* iface, ipv4_addr_t gw, uint32_t metrica, long int timeout)
{
  ripv2_route_t * route = (ripv2_route_t *) malloc(sizeof(struct ripv2_route));

  if ((route != NULL) && (subnet != NULL) && (mask != NULL) && (iface != NULL) && (gw != NULL)) {
    ...
    if (metrica<0 || metrica>16){
        fprintf(stderr, "Métrica incorrecta: ERROR en ripv2_route_create()\n");
        return NULL;
    }
    else{
        route->metrica=metrica;
        timerms_reset(&route->temporizador, timeout);
    }
...
void ripv2_route_print (ripv2_route_t * route)
{
  if (route != NULL) {
    ...
    uint32_t metrica=route->metrica;
    long int temporizador=route->temporizador;

    printf("%s/%s via %s dev %s metrica %d temporizador %ld", subnet_str, mask_str, gw_str, iface_str, metrica, temporizador);
  }
}
 ...
 */
ripv2_route_t* ripv2_route_read ( char* filename, int linenum, char * line )      //VAMOSSSSSSS POR AQUÍ
{
  ...
  int params = sscanf(line, "%s %s %s %s %u %ld\n", 
	       subnet_str, mask_str, iface_name, gw_str, &metrica, &temporizador); //revisar
  if (params != 6) {
    fprintf(stderr, "%s:%d: Invalid IPv4 Route format: '%s' (%d params)\n",
	    filename, linenum, line, params);
    fprintf(stderr, 
	    "%s:%d: Format must be: <subnet> <mask> <iface> <gw> <metrica> <temporizador>\n",
	    filename, linenum);
    return NULL;
  }
  ...
  route = ripv2_route_create(subnet, mask, iface_name, gateway, metrica, temporizador);
  if (route == NULL) {
...
ripv2_route_table_t * ripv2_route_table_create()
{
  ...
  if (table != NULL) {
    int i;
    for (i=0; i<RIPv2_ROUTE_TABLE_SIZE; i++) {  //NO sabemos cuánto vale la variable
      table->routes[i] = NULL;
    }
  }
